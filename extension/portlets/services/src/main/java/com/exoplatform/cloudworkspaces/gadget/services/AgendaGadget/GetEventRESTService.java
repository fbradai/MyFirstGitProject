...
public class GetEventRESTService extends CalendarWebservice {
  ...
  final public static String DEFAULT_EVENTCATEGORY_ID_NAME = "All";
 ...
 */
  ...
  public Response addEvent(@FormParam("calId") String calId, @FormParam("eventTitle") String eTitle, 
    ...
    if (eventType.equals(CalendarEvent.TYPE_EVENT)) event.setEventState(CalendarEvent.ST_BUSY);
    else event.setEventState(CalendarEvent.NEEDS_ACTION);
    event.setEventCategoryId(DEFAULT_EVENTCATEGORY_ID_ALL);
    event.setEventCategoryName(DEFAULT_EVENTCATEGORY_ID_NAME);
    event.setParticipant(new String[]{username});
    event.setParticipantStatus(new String[] {username + ":"});
    try {
    	if (isGroupCal) {
  		  ...
  		  DataStorage dataStorage = new JCRDataStorage(nodeHierarchyCreator, reposervice);
